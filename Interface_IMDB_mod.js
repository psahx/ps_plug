// == New Interface Script (v3 - Refined Controller Toggle + Structure Fix) ==
...
(function () {
    ...
    function component(object) { // 'object' parameter holds component config/context
        ...
        var Line = Lampa.InteractionLine; // Use Lampa's Line component
        ...
        this.build = function (data) { // Build lines/rows
            console.log("### New Interface BUILD - START"); // Removed data log for brevity
            var _this2 = this; lezydata = data; scroll.clear(); items = []; active = 0;
            ...
            if (items.length === 0) { console.warn("### New Interface BUILD - No valid lines were built."); this.empty(); return; } // Call empty if build resulted in no lines
            this.activity.loader(false); this.activity.toggle();
            console.log("### New Interface BUILD - END - Activity Toggled.");
        };

        this.buildLine = function(row_data) { // Build a single Line
             ...
             try {
                 ...
                 var line = new Line(row_data, line_params); // Create Line instance
                 // Setup Event Handlers for the Line
                 line.onFocus = (card_data)=>{ if (info && card_data?.id) { info.update(card_data); this.background(card_data); } else if (info) { info.empty(); } }; // Update info panel on card focus
                 line.onEnter = (target, card_data)=>{ console.log("### New Interface line.onEnter:", card_data?.id || card_data?.title || 'More Button'); }; // Handle select/enter
                 line.onLeft = ()=>{ console.log("### New Interface line.onLeft"); Controller.toggle('menu'); }; // Go to menu on left edge
                 line.onBack = this.back.bind(this); // Use main back handler
                 line.onDown = this.down.bind(this); // Navigate between lines
                 line.onUp = this.up.bind(this);     // Navigate between lines
                 line.create(); // Create line internals
                 scroll.append(line.render(true)); // Append line to vertical scroll
                 items.push(line); // Add to list of lines
             } catch (e) { console.error("### New Interface buildLine - ERROR creating Line for:", row_data.title, e); }
        };
        ...
        this.down = function () { // Navigate down between Lines
            if (!items.length || active >= items.length - 1) return; active++;
            active = Math.min(active, items.length - 1); if (items[active]) { console.log("### New Interface DOWN - Focusing line index:", active); items[active].toggle(); scroll.update(items[active].render(true), false); }
        };
        this.up = function () { // Navigate up between Lines
             if (!items.length || active <= 0) { Lampa.Controller.toggle('head'); return; } active--;
             if (items[active]) { console.log("### New Interface UP - Focusing line index:", active); items[active].toggle(); scroll.update(items[active].render(true), false); } else { Lampa.Controller.toggle('head'); }
        };

        // *** MODIFIED Start/Controller Toggle ***
        this.start = function () {
            ...
            Lampa.Controller.add('content', {
                ...
                toggle: function toggle() {
                    ...
                    // *** ADD CHECK: Only proceed if items exist ***
                    if (items && items.length > 0 && items[active]) {
                        console.log("### New Interface Controller TOGGLE: Toggling item", active);
                        items[active].toggle(); // Focus the currently active line
                    } else if (items && items.length > 0) {
                         console.log("### New Interface Controller TOGGLE: Active index invalid, focusing item 0");
                         active = 0; // Reset to first line if active index invalid
                         items[0].toggle();
                    } else {
                         console.log("### New Interface Controller TOGGLE: No lines ready yet, doing nothing.");
                         // Do nothing here, wait for build to complete
                    }
                },
                // Left/Right are handled inside the active Line component
                left: function left() { /* Let Line handle */ }, right: function right() { /* Let Line handle */ },
                up: this.up.bind(this), down: this.down.bind(this), back: this.back.bind(this)
            });
            Lampa.Controller.toggle('content');
        };
        // *** END MODIFIED Start/Controller Toggle ***
