// == Original Main Module with ONLY TMDB Rating Line Duplicated ==
(function () {
    'use strict';

    // --- create function (Info Panel Handler) ---
    function create() { // NOTE: Original 'create' takes no 'object' parameter based on user's first script paste
      var html;
      var timer;
      ...
      // Original 'render', 'empty', 'destroy' methods
    ...
    function component(object) {
      ...
      this.push = function () {}; // Original

      this.build = function (data) { // Original build code
        var _this2 = this;
        lezydata = data;
        info = new create(object); // Instantiates original 'create'
        info.create();
      ...
      this.append = function (element) { // Original append code
        ...
        item.onFocus = function (elem) { info.update(elem); _this3.background(elem); }; // Calls original update
        item.onHover = function (elem) { info.update(elem); _this3.background(elem); }; // Calls original update
      ...
      this.destroy = function () { /* Original destroy code */
           clearTimeout(background_timer); network.clear(); Lampa.Arrays.destroy(items); scroll.destroy(); if (info) info.destroy(); /* Calls original destroy */ if (html) html.remove(); items = null; network = null; lezydata = null; info = null; html = null;
      };
    }
    ...
    function startPlugin() {
      ...
      var new_interface = component; // Using original component
