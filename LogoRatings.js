// == Main Module with IMDB + TMDB + KP Display (Corrected Rating Style) ==
(function () {
    'use strict';
    ...
    // UNCHANGED Original Component Function
    ...
    function startPlugin() {
        // UNCHANGED startPlugin setup...
        if (!window.Lampa || !Lampa.Utils || !Lampa.Lang || !Lampa.Storage || !Lampa.TMDB || !Lampa.Template || !Lampa.Reguest || !Lampa.Api || !Lampa.InteractionLine || !Lampa.Scroll || !Lampa.Activity || !Lampa.Controller) { console.error("NewInterface StyleFix: Missing Lampa components"); return; }
        ...
        // **MODIFIED CSS** - Targeting inner divs for background/padding
        var style_id = 'new_interface_style_corrected'; // New ID for this style attempt
        if (!$('style[data-id="' + style_id + '"]').length) {
             ...
             $('style[data-id="new_interface_style_kp_state"]').remove(); // Clean up last one
            ...
            /* Original Promo/Watched hiding styles */
            ...
            .new-interface .full-start__rate {
                font-size: 1.3em;        /* Base size for calculations */
                margin-right: 0.6em;     /* Spacing between rating items */
                display: inline-flex;    /* Align inner divs horizontally */
                align-items: center;     /* Align items vertically */
                vertical-align: middle;  /* Align whole block with surrounding text */
                /* NO background/padding/border-radius on the outer container */
            }
            .new-interface .full-start__rate > div:first-child { /* Number part */
                font-weight: 600;
                background-color: rgba(0, 0, 0, 0.4); /* Darker background for number */
                color: #ffffff;
                padding: 0.2em 0.5em;
                border-radius: 3px;
                line-height: 1.3; /* Match overall font size */
            }
            .new-interface .full-start__rate > div:last-child { /* Label part */
                font-size: 0.7em;      /* Smaller text for label */
                opacity: 0.8;
                text-transform: uppercase;
                margin-left: 0.4em;   /* Space between number box and label */
                /* No background for the label */
            }
            /* --- End CORRECTED Rating Box Styles --- */
