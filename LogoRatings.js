// == Main Module | Applying Lampa Source CSS Values ==
(function () {
    'use strict';
    ...
    function startPlugin() {
        ...
        if (!window.Lampa || !Lampa.Utils || !Lampa.Lang || !Lampa.Storage || !Lampa.TMDB || !Lampa.Template || !Lampa.Reguest || !Lampa.Api || !Lampa.InteractionLine || !Lampa.Scroll || !Lampa.Activity || !Lampa.Controller) { console.error("NewInterface Apply Source CSS Final: Missing Lampa components"); return; }
        ...
        // **MODIFIED CSS**: Applying Lampa Source rules + Logo Sizing
        var style_id = 'new_interface_style_lampa_source_applied'; // ID for this state
        if (!$('style[data-id="' + style_id + '"]').length) {
            ...
            /* Original Base CSS Rules (excluding .full-start__rate itself) */
            ...
            /* Rule 1 Applied to .full-start__rate (Outer Container) */
            .full-start__rate {
                ...
                padding: 0.1em 0.4em;          /* Minimal padding inside wrapper */
                gap: 0.5em;                    /* Space between number & logo */
                overflow: hidden;
            }
            ...
            .full-start__rate > div { /* Targets the first div (Number/Score) */
                ...
                flex-shrink: 0;
                background: rgba(0, 0, 0, 0.15); /* Lampa Source background (Same as parent) */
                border-radius: 0.3em;          /* Lampa Source radius */
                display: flex;
                align-items: center;
                justify-content: center;
                font-weight: 600;              /* Added for visibility */
                color: white;                  /* Added for visibility */
                line-height: 1.2;
                font-size: 0.9em;              /* Slightly smaller relative to parent */
                order: 1;                      /* Number first */
            }
            /* Specific padding for RT score number div */
             .rt-rating-item > div.rt-score {
                 padding-left: 0.6em; /* Keep slightly wider padding for % */
                 padding-right: 0.6em;
             }
            /* Rule 3 Applied to Logo (Implicitly last child) + Sizing */
            .rating-logo {
                /* Lampa source label had: font-size: 0.7em; padding: 0 0.5em; */
                /* We apply sizing instead */
                height: 1.0em;    /* Adjusted height relative to 1.45em parent */
                width: auto;
                max-width: 38px;  /* Adjusted max-width */
                vertical-align: middle;
                order: 2;         /* Logo second */
                line-height: 0;   /* Prevent logo affecting line height */
            }
             /* Specific Logo Adjustments */
            .tmdb-logo { height: 0.7em; } /* Adjusted relative height */
            .rt-logo { height: 1.0em; }  /* Adjusted relative height */
            /* --- End Rating Box Styles --- */

            </style>
            `);
          // Use original template name 'new_interface_style' - Changed back
