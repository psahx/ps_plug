// == Main Module with IMDB + TMDB + Rotten Tomatoes (MDBList Source) Display ==
(function () {
    'use strict';

    // --- MDBList Fetcher State (GLOBAL within plugin scope) ---
    // Using new cache names for clarity
    ...
    function create() {
      ...
      this.update = function (data) {
        ...
        // Original 'update' method PART 1 - UNCHANGED
        ...
        // **MODIFIED**: Use MDBLIST_Fetcher
        // Clear previous state for this item in case of re-focus
        delete mdblistRatingsCache[data.id];
        delete mdblistRatingsPending[data.id];

        // Check if MDBLIST_Fetcher exists and fetch is not pending/cached
        if (window.MDBLIST_Fetcher && typeof window.MDBLIST_Fetcher.fetch === 'function' && data.id && data.method) { // Ensure method exists
             mdblistRatingsPending[data.id] = true;

             // Call the MDBLIST_Fetcher
             window.MDBLIST_Fetcher.fetch(data, function(mdblistResult) { // 'data' should have id and method
                 mdblistRatingsCache[data.id] = mdblistResult; // Store result from MDBLIST
                 delete mdblistRatingsPending[data.id]; // Clear pending flag
            ...
            console.warn("MDBLIST_Fetcher Integration: movieData missing 'method' property.");
        }
        // --- END MODIFIED ---


        // Original 'update' method PART 2 - UNCHANGED
        this.load(data);
        // End Original PART 2
      };
      ...
      this.draw = function (data) {
        ...
        // Get ratings result from MDBLIST_Fetcher cache
        var mdblistResult = mdblistRatingsCache[data.id];

        // 1. IMDB Rating (Always display, uses MDBList data)
        ...
        // 2. TMDB Rating (Always display, uses original TMDB data)
        details.push('<div class="full-start__rate"><div>' + vote + '</div><div>TMDB</div></div>');

        // 3. Rotten Tomatoes Rating (Display with logo if tomatometer score exists)
        // Use URLs for the PNG logos from Wikipedia
        const rtFreshLogoUrl = 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/5b/Rotten_Tomatoes.svg/40px-Rotten_Tomatoes.svg.png'; // Smaller PNG
        const rtRottenLogoUrl = 'https://upload.wikimedia.org/wikipedia/commons/thumb/5/52/Rotten_Tomatoes_rotten.svg/40px-Rotten_Tomatoes_rotten.svg.png'; // Smaller PNG
        ...
        if (mdblistResult && typeof mdblistResult.tomatometer === 'number') {
            ...
            if (score >= 60) {
                logoUrl = rtFreshLogoUrl; // Fresh Tomato
            } else if (score >= 0) { // Scores 0-59 are Rotten
                logoUrl = rtRottenLogoUrl; // Rotten Splat
            }

            // Only add the element if we have a valid logo (i.e., score >= 0)
            if (logoUrl) {
                 details.push(
                     '<div class="full-start__rate rt-rating">' + // Use base class + specific class
                       '<img src="' + logoUrl + '" class="rt-logo" alt="RT Status" draggable="false">' + // Logo
                       '<div>' + score + '%</div>' + // Percentage
                     '</div>'
                 );
            }
        }
        // KP Rating is now replaced by Rotten Tomatoes logic above.
      ...
      this.load = function (data) {
        ...
        timer = setTimeout(function () {
          ...
          network.silent(url, function (movie) {
            loaded[url] = movie;
            // Add 'method' to the loaded data if it's not present, needed by MDBList fetcher trigger
            if (!movie.method) {
                 movie.method = data.name ? 'tv' : 'movie';
            }
            _this.draw(movie);
          });
        }, 300);
      ...
      this.destroy = function () {
        ...
        loaded = {};
        html = null;
        // **MODIFIED**: Clear MDBList cache vars
        mdblistRatingsCache = {};
    ...
    function component(object) {
        ...
        this.refresh = function () { this.activity.loader(true); this.activity.need_refresh = true; }; this.pause = function () {}; this.stop = function () {}; this.render = function () { return html; }; this.destroy = function () { /* Original destroy code */ clearTimeout(background_timer); network.clear(); Lampa.Arrays.destroy(items); scroll.destroy(); if (info) info.destroy(); /* Calls modified destroy */ if (html) html.remove(); items = null; network = null; lezydata = null; info = null; html = null; };
    }


    // --- Plugin Initialization Logic ---
    function startPlugin() {
        if (!window.Lampa || !Lampa.Utils || !Lampa.Lang || !Lampa.Storage || !Lampa.TMDB) { // Added Storage/TMDB check
            console.error("NewInterface RT: Missing Lampa components");
            return;
        }
        Lampa.Lang.add({
        ...
        // Original CSS + **ADDED** RT Logo CSS
        var style_id = 'new_interface_style_with_rt'; // New unique ID
        if (!$('style[data-id="' + style_id + '"]').length) {
            Lampa.Template.add(style_id, `
            <style data-id="${style_id}">
            /* All original CSS from the first script */
            ...
            .new-interface-info__details { margin-bottom: 1.6em; display: flex; align-items: center; flex-wrap: wrap; min-height: 1.9em; font-size: 1.1em; gap: 0.5em 0; /* Added gap for spacing */ }
            .new-interface-info__split { margin: 0 0.5em; font-size: 0.7em; opacity: 0.6; } /* Adjusted margin */
            ...
            .new-interface .full-start__rate {
                ...
                border-radius: 3px;
                white-space: nowrap;
                margin-right: 0.5em; /* Add margin between items */
            }
            ...
            /* ** ADDED RT STYLES ** */
            .rt-rating {
                /* Inherits .full-start__rate styles */
                gap: 0.3em; /* Adjust gap for logo */
            }
            .rt-rating .rt-logo {
                width: 1.1em;  /* Scale with font size */
                height: 1.1em;
                vertical-align: middle; /* Align with text */
                /* margin-right: 0.3em; (Using gap now) */
            }
            .rt-rating > div { /* Style for the percentage text */
               font-weight: 600;
               font-size: 1em; /* Ensure percentage text is same size as rating number */
               line-height: 1; /* Prevent logo affecting line height */
            }
            /* ** END ADDED RT STYLES ** */
            </style>
            `);
